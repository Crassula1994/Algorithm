...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 0; tc < testCase; tc++) {
			...
			// 조건을 만족하는 정수 쌍의 개수를 저장할 변수 count 초기화
			int count = 0;
			
			// for 반복문을 사용해 각 정수의 범위를 순회
			for (int x = 1; x <= numberA; x++) {
				for (int y = 1; y <= numberB; y++) {
					for (int z = 1; z <= numberC; z++) {
						
						// 해당 정수 쌍이 조건을 만족하는 경우 조건을 만족하는 정수 쌍의 개수를 갱신
						if (x % y == y % z && y % z == z % x)
							count++;
					}
				}
			...
			// write() 메서드를 사용해 조건을 만족하는 정수 쌍의 개수를 출력
			out.write(count + "\n");
		}
