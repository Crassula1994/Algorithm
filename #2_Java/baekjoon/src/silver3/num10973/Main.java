...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// prevFinder() 메서드를 실행해 이전 순열이 존재하는지를 변수 hasPrev에 할당
		boolean hasPrev = prevFinder();
		
		// 이전 순열이 존재하는 경우
		if (hasPrev) {
			
			// for 반복문을 사용해 이전 순열을 출력
			for (int idx = 0; idx < permutation.length; idx++)
				out.write(permutation[idx] + " ");
		
		// 이전 순열이 존재하지 않는 경우 -1을 출력
		} else {
	...
	// prevFinder() 메서드 정의
	public static boolean prevFinder() {
		...
		while (originIdx > 0 && permutation[originIdx] > permutation[originIdx - 1])
			originIdx--;
		
		// 이전 순열이 존재하지 않는 경우 false 반환
		if (originIdx == 0)
			return false;
		
		// while 반복문을 사용해 다른 위치를 교환할 인덱스를 찾을 때까지 순회
		while (permutation[targetIdx] > permutation[originIdx - 1])
			targetIdx--;
