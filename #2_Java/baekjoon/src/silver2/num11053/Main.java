...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// 주어진 수열 및 해당 위치까지 가장 긴 증가하는 부분 수열의 길이를 저장할 각 배열 초기화
		...
		// 가장 긴 증가하는 부분 수열의 길이를 저장할 변수 maxLength 초기화
		int maxLength = 1;
		...
		for (int idx = 1; idx < sequence.length; idx++) {
			...
			for (int prev = 0; prev < idx; prev++) {
				...
				if (sequence[prev] < sequence[idx]) {
					...
					// max() 메서드를 사용해 가장 긴 증가하는 부분 수열의 길이를 모두 갱신
					lisLength[idx] = Math.max(lisLength[prev] + 1, lisLength[idx]);
