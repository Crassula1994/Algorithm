...
public class Main {
	...
	public static void bfs(int startNode) {
		...
		// 방문할 정점을 저장할 Queue 객체 visitList 초기화
		...
		while (!visitList.isEmpty()) {
			...
			// poll() 메서드를 사용해 방문한 정점을 변수 currentNode에 할당
			int currentNode = visitList.poll();
			
			// for 반복문을 사용해 연결된 정점을 순회
			for (int idx = 0; idx < connection.get(currentNode).size(); idx++) {
				
				// get() 메서드를 사용해 연결된 정점을 변수 nextNode에 할당
				int nextNode = connection.get(currentNode).get(idx);
				
				// 해당 정점을 방문한 적이 없는 경우 visitList에 추가 후 방문 처리
				if (visited[nextNode] == 0) {
					visitList.offer(nextNode);
					visited[nextNode] = visitCnt++;
				}
			}
		}
	}
