...
public class Main {
	...
	// 입력 받은 소의 키를 배열 형태로 저장할 배열 heights 초기화
	static int[] heights;
	
	// 입력 받은 소의 키를 세그먼트 트리 형태로 최솟값과 최댓값을 저장할 2차원 배열 segmentTree 초기화
	...
	public static void main(String[] args) throws IOException {
		...
		// nextToken() 및 parseInt() 메서드를 사용해 입력 받은 소의 수와 그룹의 수를 각 변수에 할당
		int cowNum = Integer.parseInt(st.nextToken());
		int groupNum = Integer.parseInt(st.nextToken());
		
		// 입력 받은 소의 키를 배열 형태로 저장할 배열 heights 초기화
		heights = new int[cowNum];
		
		// 입력 받은 소의 키를 세그먼트 트리 형태로 최솟값과 최댓값을 저장할 2차원 배열 segmentTree 초기화
		segmentTree = new int[cowNum * 4][2];
		
		// for 반복문을 사용해 입력 받은 소의 키를 배열 heights에 저장
		for (int idx = 0; idx < heights.length; idx++)
			heights[idx] = Integer.parseInt(in.readLine());
		
		// segmentTreeMaker() 메서드를 호출해 입력 받은 소의 키에 대하여 최솟값과 최댓값을 세그먼트 트리 형태로 저장
		segmentTreeMaker(1, 0, cowNum - 1);
		
		// while 반복문을 사용해 각 소의 그룹을 순회
		while (groupNum-- > 0) {
			
			// StringTokenizer 객체를 불러와 변수 st에 재할당
			st = new StringTokenizer(in.readLine());
			
			// nextToken() 및 parseInt() 메서드를 사용해 소의 키에 대하여 최솟값과 최댓값을 구할 범위를 각 변수에 할당
			int startIdx = Integer.parseInt(st.nextToken()) - 1;
			int endIdx = Integer.parseInt(st.nextToken()) - 1;
			
			// segmentTreeFinder() 메서드를 호출해 주어진 범위의 소의 키 최솟값과 최댓값을 배열 extremum에 저장
			int[] extremum = segmentTreeFinder(1, 0, cowNum - 1, startIdx, endIdx);
			
			// write() 메서드를 사용해 해당 구간의 가장 키가 큰 소와 가장 키가 작은 소 사이의 키 차이를 출력
			out.write((extremum[1] - extremum[0]) + "\n");
		}
	...
	public static void segmentTreeMaker(int node, int start, int end) {
		...
		if (start == end) {
			segmentTree[node][0] = heights[start];
			segmentTree[node][1] = heights[start];
			return;
		}
	...
	public static int[] segmentTreeFinder(int node, int start, int end, int startIdx, int endIdx) {
		...
		if (startIdx > end || endIdx < start)
			return new int[] {MAX_HEIGHT, MIN_HEIGHT};
