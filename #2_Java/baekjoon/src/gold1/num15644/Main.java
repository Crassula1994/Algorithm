...
public class Main {
	...
	static class History {
		...
		// 해당 위치까지의 이동 횟수 및 이동 경로를 나타낼 각 변수 초기화
		int count;
		String route;
		
		// 매개변수를 입력 받는 생성자 정의
		public History(int count, String route) {
			this.count = count;
			this.route = route;
		}
	}
	...
	public static void main(String[] args) throws IOException {
		...
		// 각 칸에 대한 두 구슬의 이동 순서를 저장할 4차원 배열 moved 초기화
		moved = new History[height][width][height][width];
		...
		// boardTilter() 메서드를 호출해 빨간 구슬을 구멍을 통해 빼낼 수 있는 최소 횟수와 경로를 변수 minHistory에 할당
		History minHistory = boardTilter();
		
		// valueOf() 및 write() 메서드를 사용해 빨간 구슬을 구멍을 통해 빼낼 수 있는 최소 횟수와 경로를 출력
		out.write(String.valueOf(minHistory.count + "\n" + minHistory.route));
	...
	// boardTilter() 메서드 정의
	public static History boardTilter() {
		...
		moved[startRed.row][startRed.column][startBlue.row][startBlue.column] = new History(0, "");
		...
		while (!moveList.isEmpty()) {
			...
			// 각 구슬의 현재 위치 및 이동 횟수와 경로를 각 변수에 할당
			Location curRed = curLocation[0];
			Location curBlue = curLocation[1];
			History curHistory = moved[curRed.row][curRed.column][curBlue.row][curBlue.column];
			int curCount = curHistory.count;
			String curRoute = curHistory.route;
			...
			outer: for (int d = 0; d < 4; d++) {
				...
				while (board[nextRedR + dr[d]][nextRedC + dc[d]] != '#') {
					...
					// 빨간 구슬이 구멍에 빠지는 경우 이동 횟수 및 이동 경로를 반환
					if (board[nextRedR][nextRedC] == 'O')
						return new History(curCount + 1, curRoute + direction[d]);
				}
				...
				moved[nextRedR][nextRedC][nextBlueR][nextBlueC] = new History(curCount + 1, curRoute + direction[d]);
			}
		}
		
		// 10번 이하로 움직여 빨간 구슬을 빼낼 수 없는 경우 -1과 빈 경로를 반환
		return new History(-1, "");
	}
}