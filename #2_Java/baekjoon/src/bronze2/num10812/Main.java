...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// nextToken() 및 parseInt() 메서드를 사용해 입력 받은 바구니의 수 및 바구니의 순서를 회전할 횟수를 각 변수에 할당
		...
		// for 반복문을 사용해 각 바구니의 순서를 회전할 횟수를 순회
		for (int cnt = 0; cnt < changeCnt; cnt++) {
			
			// StringTokenizer 객체를 불러와 변수 st에 재할당
			st = new StringTokenizer(in.readLine());
			
			// nextToken() 및 parseInt() 메서드를 사용해 바구니의 순서를 회전하기 위해 필요한 바구니 번호를 각 변수에 할당
			int startBkt = Integer.parseInt(st.nextToken());
			int endBkt = Integer.parseInt(st.nextToken());
			int midBkt = Integer.parseInt(st.nextToken());
			
			// clone() 메서드를 사용해 바꾼 결과를 저장할 배열 temp 초기화
			int[] temp = baskets.clone();
			
			// for 반복문을 사용해 처음부터 중간까지의 바구니의 순서를 바꾸어 저장
			for (int i = startBkt - 1, j = startBkt + endBkt - midBkt; i < midBkt - 1; i++)
				temp[j++] = baskets[i];
			
			// for 반복문을 사용해 중간부터 끝까지의 바구니의 순서를 바꾸어 저장
			for (int i = midBkt - 1, j = startBkt - 1; i < endBkt; i++)
				temp[j++] = baskets[i];
