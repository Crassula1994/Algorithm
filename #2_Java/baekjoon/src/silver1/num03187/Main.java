...
public class Main {
	...
	// 영역의 세로 및 가로 길이, 살아남은 양과 늑대의 수를 저장할 각 변수 초기화
	static int height;
	static int width;
	...
	// 영역의 상태, 해당 영역을 확인했는지 여부를 나타낼 각 2차원 배열 초기화
	static char[][] field;
	...
	public static void main(String[] args) throws IOException {
		...
		// nextToken() 및 parseInt() 메서드를 사용해 입력 받은 영역의 세로 및 가로 길이를 각 변수에 할당
		...
		// for 반복문을 사용해 입력 받은 영역의 상태를 2차원 배열 field에 저장
		for (int r = 0; r < height; r++)
			field[r] = in.readLine().toCharArray();
		
		// for 반복문을 사용해 입력 받은 영역의 각 칸을 순회
		for (int r = 0; r < height; r++) {
	...
	public static void sheepAttacker(int startRow, int startCol) {
		...
		while (!checkList.isEmpty()) {
			...
			if (field[curLoc[0]][curLoc[1]] == 'k') {
				sheepCnt++;
				
			// 해당 위치에 늑대가 존재하는 경우 현재 영역 내 늑대의 수를 갱신
			} else if (field[curLoc[0]][curLoc[1]] == 'v') {
				wolfCnt++;
			}
			...
			for (int d = 0; d < 4; d++) {
				...
				// 해당 위치가 영역의 범위를 벗어난 경우 다음 위치를 순회
				if (nr < 0 || nr >= height || nc < 0 || nc >= width)
					continue;
				
				// 해당 위치가 울타리이거나 이미 확인한 곳인 경우 다음 위치를 순회
				if (field[nr][nc] == '#' || checked[nr][nc])
					continue;
