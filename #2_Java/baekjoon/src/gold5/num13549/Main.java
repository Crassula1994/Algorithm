...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		if (startLoc >= targetLoc) {
			...
			// valueOf() 및 write() 메서드를 사용해 왼쪽으로 한 칸씩 이동한 횟수를 출력
			out.write(String.valueOf(startLoc - targetLoc));
	...
	public static void timeCalculator(int startLoc, int targetLoc) {
		...
		while (!visitList.isEmpty()) {
			...
			// 수빈이가 왼쪽으로 한 칸 이동해도 범위를 벗어나지 않고 해당 위치를 방문하지 않았거나 더 적은 시간에 방문한 경우
			...
			// 수빈이가 오른쪽으로 한 칸 이동해도 범위를 벗어나지 않고 해당 위치를 방문하지 않았거나 더 적은 시간에 방문한 경우
			...
			// 수빈이가 오른쪽으로 두 배 이동해도 범위를 벗어나지 않고 해당 위치를 방문하지 않았거나 더 적은 시간에 방문한 경우
			if (currentLoc * 2 < location.length && (location[currentLoc * 2] == 0 || location[currentLoc * 2] > location[currentLoc])) {
				
				// 해당 위치에 소요 시간을 갱신
				location[currentLoc * 2] = location[currentLoc];
