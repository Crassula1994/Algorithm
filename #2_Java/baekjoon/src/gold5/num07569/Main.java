...
public class Main {
	...
	// 창고의 가로와 세로의 크기 및 높이를 나타낼 각 변수 초기화
	static int width;
	static int depth;
	static int height;
	...
	public static void main(String[] args) throws IOException {
		...
		// nextToken() 및 parseInt() 메서드를 사용해 입력 받은 창고의 가로와 세로 칸 수 및 높이를 각 변수에 할당
		...
		for (int h = 0; h < height; h++) {
			...
			for (int r = 0; r < depth; r++) {
				...
				for (int c = 0; c < width; c++) {
					warehouse[h][r][c] = Integer.parseInt(st.nextToken());
					
					// 토마토가 하나라도 익지 않은 경우 isRipe 변수 갱신
					if (isRipe && warehouse[h][r][c] == 0)
						isRipe = false;
	...
	public static int timeCalculator() {
		...
		while (!visitList.isEmpty()) {
			...
			for (int d = 0; d < 6; d++) {
				...
				// 인접한 위치를 각 변수에 할당
				int nh = currentLoc[0] + dh[d];
				int nr = currentLoc[1] + dr[d];
				int nc = currentLoc[2] + dc[d];
				
				// 인접한 위치가 배열을 벗어나지 않고 익지 않은 경우
				if (nh >= 0 && nh < height && nr >= 0 && nr < depth && nc >= 0 && nc < width && warehouse[nh][nr][nc] == 0) {
					
					// 해당 위치를 visitList에 추가
					int[] nextLoc = {nh, nr , nc};
					visitList.add(nextLoc);
					
					// 익은 날짜를 갱신
					warehouse[nh][nr][nc] = warehouse[currentLoc[0]][currentLoc[1]][currentLoc[2]] + 1;
				}
			}
		}
		...
		// for 반복문을 사용해 토마토 창고의 높이, 행과 열을 순회
		for (int h = 0; h < height; h++) {
			for (int r = 0; r < depth; r++) {
				for (int c = 0; c < width; c++) {
					...
					minTime = (warehouse[h][r][c] > minTime) ? warehouse[h][r][c] : minTime;
				}
			}
		}
