...
public class Main {
	...
	// 수빈이가 동생을 가장 빠른 방법으로 찾는 방법의 가짓수를 저장할 변수 count 초기화
	static int count = 0;
	...
	public static void main(String[] args) throws IOException {
		...
		// 수빈이와 동생이 같은 위치에 있거나 동생이 수빈이보다 왼쪽에 있는 경우
		if (startLoc >= targetLoc) {
			
			// write() 메서드를 사용해 왼쪽으로 한 칸씩 이동한 횟수 및 방법의 수를 출력
			out.write((startLoc - targetLoc) + "\n" + 1);
		...
		// 동생이 수빈이보다 오른쪽에 있는 경우
			...
			// write() 메서드를 사용해 동생을 찾는 최단 시간 및 해당 방법의 수를 출력
			out.write(location[targetLoc] - 1 + "\n" + count);
		}
	...
	public static void timeCalculator(int startLoc, int targetLoc) {
		...
		while (!visitList.isEmpty()) {
			...
			// 동생을 찾은 경우 가장 빠른 방법으로 찾는 방법의 가짓수를 갱신
			if (currentLoc == targetLoc)
				count++;
			
			// 이미 동생을 최단 시간에 찾은 경우 반복문 탈출
			if (location[targetLoc] != 0 && location[currentLoc] > location[targetLoc])
				break;
			
			// 수빈이가 왼쪽으로 한 칸 이동해도 범위를 벗어나지 않고 해당 위치를 방문하지 않았거나 같은 시간에 방문한 경우
			if (currentLoc - 1 >= 0 && (location[currentLoc - 1] == 0 || location[currentLoc - 1] == location[currentLoc] + 1)) {
			...
			// 수빈이가 오른쪽으로 한 칸 이동해도 범위를 벗어나지 않고 해당 위치를 방문하지 않았거나 같은 시간에 방문한 경우
			if (currentLoc + 1 < location.length && (location[currentLoc + 1] == 0 || location[currentLoc + 1] == location[currentLoc] + 1)) {
			...
			if (currentLoc * 2 < location.length && (location[currentLoc * 2] == 0 || location[currentLoc * 2] == location[currentLoc] + 1)) {
