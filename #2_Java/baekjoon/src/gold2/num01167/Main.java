...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int n = 0; n < nodeNum; n++) {
			...
			// nextToken() 및 parseInt() 메서드를 사용해 현재 정점의 번호를 변수 startNode에 할당
			int startNode = Integer.parseInt(st.nextToken());
			
			// while 반복문을 사용해 각 간선의 정보를 순회
			while (true) {
				
				// nextToken() 및 parseInt() 메서드를 사용해 연결된 정점의 번호를 변수 endNode에 할당
				int endNode = Integer.parseInt(st.nextToken());
				
				// 연결된 정점의 번호가 -1인 경우 반복문 탈출
				if (endNode == -1)
					break;
				
				// nextToken() 및 parseInt() 메서드를 사용해 정점까지의 거리를 변수 distance에 할당
		...
		// farthestNodeFinder() 메서드를 호출해 1번 노드로부터 가장 멀리 떨어진 노드를 갱신
		visited[1] = true;
